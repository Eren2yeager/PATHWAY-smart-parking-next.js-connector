...
import { z } from 'zod';
import { AlertCircle, Loader2, Calendar as CalendarIcon, DollarSign, Users } from 'lucide-react';
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Card>
        <CardContent className="p-6 space-y-4">
          <h3 className="text-lg font-bold text-gray-900 dark:text-[#e5e7eb] mb-4">Basic Information</h3>

          {/* Contractor Name */}
          <div>
            <label htmlFor="name" className="block text-sm font-medium text-gray-700 dark:text-[#d1d5db] mb-2">
              Contractor Name *
            </label>
            <Input
              type="text"
              ...
              className={errors.name ? 'border-red-500 dark:border-red-700' : ''}
          ...
          {/* Contact Person */}
          <div>
            <label htmlFor="contactPerson" className="block text-sm font-medium text-gray-700 dark:text-[#d1d5db] mb-2">
              Contact Person *
            </label>
            <Input
              type="text"
              ...
              className={errors.contactPerson ? 'border-red-500 dark:border-red-700' : ''}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label htmlFor="email" className="block text-sm font-medium text-gray-700 dark:text-[#d1d5db] mb-2">
                Email *
              </label>
              <Input
                type="email"
                ...
                className={errors.email ? 'border-red-500 dark:border-red-700' : ''}
            ...
            <div>
              <label htmlFor="phone" className="block text-sm font-medium text-gray-700 dark:text-[#d1d5db] mb-2">
                Phone *
              </label>
              <Input
                type="tel"
                ...
                className={errors.phone ? 'border-red-500 dark:border-red-700' : ''}
          ...
          {/* Status */}
          <div>
            <label htmlFor="status" className="block text-sm font-medium text-gray-700 dark:text-[#d1d5db] mb-2">
              Status *
            </label>
            <Select
            ...
            >
              <SelectTrigger className={`w-full ${errors.status ? 'border-red-500 dark:border-red-700' : ''}`}>
      ...
      <Card>
        <CardContent className="p-6 space-y-4">
          <div className="flex items-center gap-2 mb-4">
            <CalendarIcon className="w-5 h-5 text-gray-500 dark:text-[#71717a]" />
            <h3 className="text-lg font-bold text-gray-900 dark:text-[#e5e7eb]">Contract Details</h3>
          </div>
            ...
            <div>
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className={`w-full justify-between text-left font-normal ${
                      !formData.contractDetails.startDate ? 'text-muted-foreground' : ''
                    } ${errors['contractDetails.startDate'] ? 'border-red-500 dark:border-red-700' : ''}`}
                  >
                    {formData.contractDetails.startDate ? (
                      format(new Date(formData.contractDetails.startDate), 'PPP')
                    ) : (
                ...
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
                    mode="single"
                    selected={formData.contractDetails.startDate ? new Date(formData.contractDetails.startDate) : undefined}
                    onSelect={(date) => {
                      if (date) {
                        handleChange('contractDetails.startDate', format(date, 'yyyy-MM-dd'));
                      }
                    }}
                    defaultMonth={formData.contractDetails.startDate ? new Date(formData.contractDetails.startDate) : undefined}
                  />
                </PopoverContent>
              </Popover>
              {errors['contractDetails.startDate'] && (
                <p className="mt-1 text-sm text-red-600 dark:text-red-400">{errors['contractDetails.startDate']}</p>
              )}
            </div>

            <div>
              <label htmlFor="endDate" className="block text-sm font-medium text-gray-700 dark:text-[#d1d5db] mb-2">
                End Date *
              </label>
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    className={`w-full justify-between text-left font-normal ${
                      !formData.contractDetails.endDate ? 'text-muted-foreground' : ''
                    } ${errors['contractDetails.endDate'] ? 'border-red-500 dark:border-red-700' : ''}`}
                  >
                    {formData.contractDetails.endDate ? (
                      format(new Date(formData.contractDetails.endDate), 'PPP')
                    ) : (
                      <span>Pick a date</span>
                    )}
                    <CalendarIcon className="w-4 h-4 opacity-50" />
                ...
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
                    mode="single"
                    selected={formData.contractDetails.endDate ? new Date(formData.contractDetails.endDate) : undefined}
                    onSelect={(date) => {
                      if (date) {
                        handleChange('contractDetails.endDate', format(date, 'yyyy-MM-dd'));
                      }
                    }}
                    defaultMonth={formData.contractDetails.endDate ? new Date(formData.contractDetails.endDate) : undefined}
                    disabled={(date: Date) => {
                      // Disable dates before start date
                      if (formData.contractDetails.startDate) {
                        return date < new Date(formData.contractDetails.startDate);
                      }
                      return false;
                    }}
                  />
              ...
              {errors['contractDetails.endDate'] && (
                <p className="mt-1 text-sm text-red-600 dark:text-red-400">{errors['contractDetails.endDate']}</p>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <Input
              ...
              className={errors['contractDetails.allocatedCapacity'] ? 'border-red-500 dark:border-red-700' : ''}
              placeholder="e.g., 100"
            />
          ...
          <div>
            ...
            <Input
              ...
              className={errors['contractDetails.penaltyPerViolation'] ? 'border-red-500 dark:border-red-700' : ''}
