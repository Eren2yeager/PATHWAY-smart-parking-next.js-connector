...
export default function ActivityFeed({ limit = 10 }: ActivityFeedProps) {
  ...
  const fetchActivities = async () => {
    try {
      ...
      const recordActivities: ActivityItem[] = recordsData.data.map(
        (record: VehicleRecord) => {
          ...
          return {
            ...
            color: isExit
              ? "bg-blue-100 text-blue-600 dark:bg-[#4f46e5]/20 dark:text-[#818cf8]"
              : "bg-green-100 text-green-600 dark:bg-[#059669]/20 dark:text-[#34d399]",
          };
        },
      );
      ...
      const violationActivities: ActivityItem[] = violationsData.data.map(
        (violation: Violation) => ({
          ...
          color: "bg-red-100 text-red-600 dark:bg-[#dc2626]/20 dark:text-[#f87171]",
        }),
      );
  ...
  if (loading) {
    return (
      <div className="bg-white dark:bg-[#111316] rounded-lg shadow dark:shadow-none border border-gray-200 dark:border-[#2a2e37] p-6">
        <h3 className="text-lg font-bold text-gray-900 dark:text-[#e5e7eb] mb-4">
          Recent Activity
        </h3>
        <div className="space-y-3">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="flex items-start space-x-3 animate-pulse">
              <div className="w-8 h-8 bg-gray-200 dark:bg-[#2a2e37] rounded-full"></div>
              <div className="flex-1">
                <div className="h-4 bg-gray-200 dark:bg-[#2a2e37] rounded w-3/4 mb-2"></div>
                <div className="h-3 bg-gray-200 dark:bg-[#2a2e37] rounded w-1/2"></div>
              </div>
            </div>
          ))}
        </div>
    ...
    return (
      <div className="bg-white dark:bg-[#111316] rounded-lg shadow dark:shadow-none border border-gray-200 dark:border-[#2a2e37] p-6">
        ...
        <div className="text-center py-8">
          <p className="text-red-600 dark:text-[#f87171]">{error}</p>
        </div>
      </div>
    );
  }
  ...
  if (activities.length === 0) {
    return (
      <div className="bg-white dark:bg-[#111316] rounded-lg shadow dark:shadow-none border border-gray-200 dark:border-[#2a2e37] p-6">
        ...
        <div className="text-center py-8">
          <p className="text-gray-500 dark:text-[#9ca3af]">No recent activity</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white dark:bg-[#111316] rounded-lg shadow dark:shadow-none border border-gray-200 dark:border-[#2a2e37] p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-bold text-gray-900 dark:text-[#e5e7eb]">Recent Activity</h3>
        <button
      ...
      <div className="space-y-4">
        {timeGroups.map((group) => {
          ...
          return (
            <div key={group}>
              <h4 className="text-xs font-semibold text-gray-500 dark:text-[#71717a] uppercase tracking-wider mb-2">
                {group}
              </h4>
              <div className="space-y-3">
                {groupActivities.map((activity) => (
                  <div key={activity.id} className="flex items-start space-x-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-gray-900 dark:text-[#e5e7eb] truncate">
                        {activity.description}
                      </p>
                      <div className="flex items-center text-xs text-gray-500 dark:text-[#9ca3af] mt-1">
