...
import { requireAuth, requireRole, handleAuthError } from '@/lib/auth/auth';
import { updateSettingsSchema } from '@/lib/db/schemas';
  ...
  try {
    ...
    if (!settings) {
      const defaultSettings = {
        ...
        pythonBackend: {
          httpUrl: process.env.NEXT_PUBLIC_PATHWAY_BACKEND_URL || process.env.NEXT_PUBLIC_PYTHON_BACKEND_URL || 'http://localhost:8000',
          wsUrl: process.env.NEXT_PUBLIC_PATHWAY_BACKEND_WS_URL || process.env.NEXT_PUBLIC_PYTHON_BACKEND_WS_URL || 'ws://localhost:8000',
        },
        cameras: {
          gateFrameSkip: 2,
          lotFrameSkip: 5,
