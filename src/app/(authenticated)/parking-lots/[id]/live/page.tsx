...
export default function LiveMonitorPage() {
  ...
  const [signalingUrl] = useState(process.env.NEXT_PUBLIC_PATHWAY_BACKEND_WS_URL || process.env.NEXT_PUBLIC_PYTHON_BACKEND_WS_URL || 'ws://localhost:8000');
              ...
              try {
                ...
                await ws.connect(
                  async (data) => {
                    ...
                    if (data.event_type === 'plate_detected' && data.plate_number && !data.plates) {
                      data = { ...data, type: 'plate_detection', plates_detected: 1, plates: [{ plate_number: data.plate_number, confidence: data.confidence ?? 0, bbox: data.bbox ?? { x1: 0, y1: 0, x2: 0, y2: 0 } }] };
                    }
                    setLastDetection(data);

                    const isPathwayPlate = data.event_type === 'plate_detected' && data.plate_number;
                    const isLegacyPlate = data.type === 'plate_detection' && (data.plates_detected ?? 0) > 0;

                    if (isPathwayPlate) {
                      addLog(`🚗 Plate: ${data.plate_number} (Pathway → DB via webhook)`);
                    ...
                    const isCapacityUpdate = data.type === 'capacity_update' || data.event_type === 'capacity_update';
                    if (isCapacityUpdate) {
    ...
    canvas.toBlob((blob) => {
      ...
      reader.onload = () => {
        ...
        wsRef.current?.sendFrame(base64, {
          ...
          parkingLotId: lotId,
          camera_id: cameraType === 'gate' ? parkingLot?.gateCamera?.id : parkingLot?.lotCamera?.id,
